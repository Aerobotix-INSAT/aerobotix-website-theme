...
$avatar = get_avatar($post->post_author, 60, '', '', ['class' => "h-auto max-w-full"]);
    ...
    <div class="w-full h-auto max-w-lg md:h-full md:w-auto md:max-w-[45vw]">
        <a href=<?php echo $permalink ?> class="block w-full h-full rounded-xl overflow-hidden md:rounded-l-none">
            ...
            if ($has_post_thumbnail) {
                ...
                echo '<img class="h-auto max-w-full" src="' . get_template_directory_uri() . '/assets/images/red-gradient.jpg" alt="Placeholder image">';
            }
            ?>
        </a>
    </div>
    <div class="max-w-lg px-3 flex flex-col gap-2 md:w-full md:max-w-none">
        ...
        <div class="font-medium w-full text-base">
            ...
            if ($tags) {
                echo 'Tags: ';
                foreach ($tags as $tag) { ?>
                    <a class="m-[1px] px-1 glass text-sm rounded" href=<?php echo get_tag_link($tag->term_id) ?>>#<?php echo str_replace(' ', '_', strtoupper($tag->name)) ?></a>
            <?php }
            }
            ?>
        </div>
