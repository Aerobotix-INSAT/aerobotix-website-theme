...
$thumbnail = get_the_post_thumbnail($post_id, '', ['class' => 'object-cover w-full h-48']);
$author = get_the_author_meta('display_name', $post->post_author);
$author_link = get_author_posts_url($post->post_author);
$date = get_the_date('F j, Y', $post_id);
$tags = get_the_tags($post_id);
$avatar = get_avatar($post->post_author, 60, '', '', ['class' => "object-cover w-9 h-9"]);
...
<div class="glass h-fit w-80 rounded-3xl p-5 flex  flex-col items-center gap-5 overflow-hidden">
    ...
    <a href=<?php echo $permalink ?> class="block w-full h-48 rounded-xl overflow-hidden">
        ...
        if ($has_post_thumbnail) {
            ...
            echo '<img class="object-cover h-48 w-96" src="' . get_template_directory_uri() . '/assets/images/red-gradient.jpg" alt="Placeholder image">';
        }
        ?>
    </a>
</div>